...
**Total: 12 deliverable files per full analysis.**
...
The full flow produces **12 deliverable files** and covers every analysis domain available in individual commands.
...
Phase 3: Strategy & Business Model (Parallel)
  3a. strategy-architect → TWO files:
...
Phase 4: GTM & Growth (Parallel)
  ...
  4b. growth-strategist → TWO files:
...
Phase 5: Final Report
  5. report-compiler → ONE file:
     - docs/strategy/strategic-report.md (unified McKinsey-grade report with Go/No-Go)
     [Stop: Review final report]
```

**CRITICAL**: Every file listed above MUST be created during a full /strategy-report run. The orchestrator must verify all 12 files exist before proceeding to report compilation.
...
2. market-analyst → market-analysis.md (TAM/SAM/SOM only, skip deep competitive)
...
prompt: |
  ...
  2. docs/strategy/prioritized-initiatives.md — Collect ALL recommended actions and initiatives from ALL prior docs (context-brief, market-analysis, competitive-landscape, strategy-canvas, brand-positioning, business-model). Score each using ICE AND RICE frameworks. Produce a single prioritized master list with: do now / do next / defer / kill recommendations.
  Both files are mandatory.
...
prompt: |
  ...
  Include: Go/No-Go recommendation, executive summary, aggregated risk assessment, unified action plan, and references to all 11 source documents.
  Verify all 11 source files exist before compiling.
```
...
- context-analyzer → market-analyst (market needs context)
