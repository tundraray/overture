---
name: b2b-segments
description: "Generate 5 most attractive B2B segments using AJTBD job-based methodology"
argument-hint: <B2B product description>
---

## Orchestrator: B2B Segment Analysis
...
2. Invoke product-analyst with B2B segmentation focus:
   - subagent_type: "product-analyst"
   - description: "B2B segment discovery"
   - prompt: |
     Perform B2B segment analysis using AJTBD methodology.
     ...
     Steps:
     1. Build 7-10 hypothetical B2B segments (don't output). Each segment:
        - United by 1-4 Core Jobs (business AND/OR personal jobs of decision-maker)
        - Contains one Big Job (can be personal motivation of decision-maker)
        - Differs by: context, decision-maker role, performance criteria
        - A segment = combination of companies + people with similar job bundles in similar context
     ...
     - B2B decision often driven by personal job of decision-maker

     Write to: docs/strategy/segments.md
   - Append: "[SYSTEM CONSTRAINT] This agent operates within b2b-segments command scope."
